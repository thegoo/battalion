...
## Follow-up Evidence: Remove Public Assess Command / Bare Invocation UX

Review blockers were addressed in the local `remove-public-assess-command` slice.

Focused failing-test evidence was captured before implementation:

```text
python3 -m pytest tests/test_cli.py -q -k "cli_help or removed_commands or no_arguments or unknown_single_word or unquoted_multi_token"
5 failed, 187 deselected
```

The expected failures proved that help exposed `assess`, no-argument and unsupported single-token input produced argparse noise, `assess` still ran publicly, and unquoted multi-token junk such as `battalion foo bar` was accepted as mission text and created `.battalion` artifacts.

Final focused result:

```text
python3 -m pytest tests/test_cli.py -q -k "bare_requirement or quoted_multiline or reserved_commands or public_docs or cli_help or removed_commands or no_arguments or unknown_single_word or unquoted_multi_token or cli_ux"
...
```text
python3 -m pytest -q
195 passed in 24.39s
```

The authoritative dogfood Plan was regenerated by Battalion from structured `human_answers` in `.battalion/ledger.yaml`; `.battalion/mission-plan.md` was not manually patched. The regenerated Plan now includes strict R-002/R-005 semantics:

- `battalion "My requirement."` is the only supported mission-start path.
- No arguments, unsupported single-token input, and unquoted multi-token junk produce clear errors.
- Quoted and multiline natural-language mission requirements are accepted.
- Existing explicit subcommands such as `review`, `status`, `plan`, and `evidence-report` remain callable.
- `assess` is removed from public routing and help; assessment remains internal workflow logic only.

Public README/help guidance no longer exposes `battalion assess` as a user path.
